...
% For the new MedPC system (VideoFreeze), we added a new timestamp labeled 'event 4', which marks the beginning of the task. In other words, 
% electrophysiological recording always starts first, followed by the activation of the MedPC system
data.events{1,2} = data.events{2,1}(1,1);

% For some reason, the timestamps for the CS are duplicated.
data.events{2,1} = data.events{2,1}(3:2:end,:);
...
% Organizing accordingly Optogenetic Stim Conterbalanced Order -> Habituation Session. 4 Hz was not used later.
